...
const KanbanColumn: React.FC<KanbanColumnProps> = ({ title, count, tasks, addNewTask, columnId }) => {
  return (
    <div className="flex-1 min-w-80">
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center space-x-2">
          <h2 className="font-medium text-gray-700 dark:text-gray-300">{title}</h2>
          ...
          >
            ...
            {title === "Done" && tasks.length === 0 && (
              <div className="border-2 border-dashed border-gray-200 dark:border-gray-600 rounded-lg p-8 text-center hover:border-gray-300 dark:hover:border-gray-500 transition-colors cursor-pointer">
                <span className="text-gray-400 dark:text-gray-500">Drag your task here...</span>
              </div>
            )}
          </div>
        )}
